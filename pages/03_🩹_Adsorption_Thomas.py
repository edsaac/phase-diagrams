...
from equations import adsorption_thomas
...
def plot_stream_matplotlib(initial_condition, parameters):
    ...
    dc, dq = adsorption_thomas([c, q], None, parameters)

    ## Calculate trayectories given initial condition
    tray = odeint(adsorption_thomas, initial_condition, time, args=(parameters,))
    c_t, q_t = tray.T  # <-- Timeseries

    ## Calculate steady-state
    sol = root(adsorption_thomas, [c_t[-1], q_t[-1]], args=(None, parameters))
    ssx, sss = sol.x
        ...
        st.latex(R"""
            \begin{equation*}
                ...
                \begin{array}{rcl}
                    ...
                    +
                    \underbrace{k_{det} \phantom{\dfrac{}{}} q}_{\textsf{Desorption}}   
